    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # ACP and Claude CLI clients communicate via local subprocesses
                # and return a plain SimpleNamespace, not an iterable stream.
                # Key on the generic `acp://` scheme so every ACP client is
                # excluded, while retaining the fork's Claude CLI transport.
                elif (
